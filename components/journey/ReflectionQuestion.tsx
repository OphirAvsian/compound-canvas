...
export function ReflectionQuestion({
  ...
  if (satisfied) {
    return (
      <div className="rounded-2xl border border-[#cfe5d9] bg-[#f0f8f4] p-4">
        <div className="flex items-center gap-2 text-[14px] font-semibold text-[#2d6b51]">
  ...
  return (
    <div className="rounded-2xl border border-[#deddd7] bg-white p-4 shadow-sm">
      ...
      <p className="mt-4 text-[16px] font-semibold leading-6">{definition.question}</p>
      <div className="mt-3 space-y-2">
        {definition.options.map((option) => (
          <label
            key={option.id}
            className={`flex min-h-12 cursor-pointer items-center gap-3 rounded-xl border px-4 py-3 text-[13px] leading-5 ${
              answerId === option.id
      ...
      {feedback && (
        <p className="mt-3 rounded-xl bg-[#f2f5f3] p-4 text-[13px] leading-6 text-[#596a61]">
          {feedback}
        </p>
      )}
      <div className="mt-3 flex flex-wrap gap-2">
        <button
          onClick={submit}
          disabled={!answerId}
          className="min-h-11 rounded-xl bg-ink px-4 py-2.5 text-[13px] font-semibold text-white disabled:cursor-not-allowed disabled:opacity-45"
        >
          Check and continue
        </button>
        <button
          ...
          className="inline-flex min-h-11 items-center gap-1.5 rounded-xl px-3 py-2.5 text-[13px] font-semibold text-[#727d77] hover:bg-[#f2f1ed]"
        >
