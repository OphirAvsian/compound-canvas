  ...
  return (
    <div className="rounded-2xl border border-[#d9d8d2] bg-white p-4 sm:p-5">
      <div>
        ...
        <h3 className="mt-1 text-[15px] font-semibold tracking-[-0.02em]">
          Where each statement comes from
        </h3>
        <p className="mt-1 max-w-[650px] text-[10px] leading-4 text-[#6c7771]">
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h4 className="text-[11px] font-semibold">{category.title}</h4>
                    <span className="rounded-full bg-white/70 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wide">
                      {records.length + assumptions.length} records
                    </span>
                  </div>
                  <p className="mt-0.5 text-[9px] leading-4 opacity-80">
              ...
              <div className="mt-3 space-y-2">
                {records.map((record) => (
                  ...
                  >
                    <p className="text-[10px] font-semibold">{record.label}</p>
                    <p className="mt-0.5 text-[9px] leading-4 opacity-80">
                      {record.source}
                      {record.method ? ` · ${record.method}` : ""}
                    </p>
                    {record.tool && (
                      <p className="mt-1 text-[8px] leading-3 opacity-70">
                ...
                {assumptions.map((assumption) => (
                  <div
                    key={assumption.id}
                    className="rounded-lg border border-current/10 bg-white/70 p-2.5 text-[9px] leading-4"
                  >
                    {assumption.statement}
                  </div>
                ))}
                {records.length === 0 && assumptions.length === 0 && (
                  <p className="rounded-lg border border-dashed border-current/20 p-2.5 text-[9px] opacity-70">
